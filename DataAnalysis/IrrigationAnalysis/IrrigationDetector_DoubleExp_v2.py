...
def jacobian(x,ti,yi):
    J = np.array([-1 * x[2] * ti * np.exp(x[0]*ti), -1 * x[3] * ti * np.exp(x[1]*ti), -1 * np.exp(x[0]*ti), -1 * np.exp(x[1]*ti), -1 + 0*ti])
...
def doubleExponentialFunction(ti, x):
    return x[2] * np.exp(x[0]*ti) + x[3] * np.exp(x[1]*ti) + x[4]


def generateTrainingSample(noSamples, windowSz):
    x_train = np.ndarray(shape=(noSamples, windowSz), dtype=float, order='F')
    y_train = np.ndarray(shape=(noSamples, 5), dtype=float, order='F')
    
    for i in range(0,noSamples):
        l1 = -1 * 0.5*np.asscalar(np.random.rand(1,1))
        l2 = -1 * (0.5*np.asscalar(np.random.rand(1,1)) + 0.5)
        dt1 = -1 * 6*np.asscalar(np.random.rand(1,1))
        dt2 = -1 * 6*np.asscalar(np.random.rand(1,1))
        offset = 28 + (4*np.asscalar(np.random.rand(1,1)) - 2)
        #y_train[i,:] = np.array([dt1, dt2, l1, l2])
        y =  np.array([l1, l2, dt1, dt2, offset])
        y_train[i,:] = y
...
def scaleParameters(param):
    scalingFactor = np.array([-0.5, -1, -6, -6, 32])
    param /= scalingFactor
    return param
...
def main():
    ...
    model.add(Dense(5, activation='linear'))#, W_regularizer=regulPen)) % 4
