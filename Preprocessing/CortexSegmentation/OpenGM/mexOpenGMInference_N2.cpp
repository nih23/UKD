...
#define SMOOTHNESS_PENALTY prhs[1]
    ...
    for(size_t y = 0; y < n_col; ++y) {
        for(size_t x = 0; x < n_row; ++x) {
            ...
            if(x + 2 < n_row) { // (x, y) -> (x + 1, y)
                size_t variableIndices[] = {variableIndex(x, y, n_col), variableIndex(x + 2, y, n_col)};
                std::sort(variableIndices, variableIndices + 2);
                energy.addFactor(fidPotts, variableIndices, variableIndices + 2);
            }
            if(y + 2 < n_col) { // (x, y) -> (x, y + 1)
                size_t variableIndices[] = {variableIndex(x, y, n_col), variableIndex(x, y + 2, n_col)};
                std::sort(variableIndices, variableIndices + 2);
                energy.addFactor(fidPotts, variableIndices, variableIndices + 2);
            }
        }
